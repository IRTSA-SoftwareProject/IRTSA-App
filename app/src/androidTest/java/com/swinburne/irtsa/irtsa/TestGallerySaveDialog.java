...
public class TestGallerySaveDialog {
  /**
  ...
  /**
   * Navigate to the GallerySaveDialog.
   */
  @Before
  public void setup(){
  ...
  public void dialogTitleIsCorrect() {
    onView(withText("Save thermogram to local storage?")).check(matches(isDisplayed()));
  }
  ...
  /**
   * Assert that the yes button closes the dialog and display's the GalleryDetail Fragment.
   */
  @Test
  public void yesButtonClosesDialogShowsGalleryDetailFragment() {
    onView(withId(R.id.galleryDetailImage)).check(doesNotExist());
    onView(withText("YES")).check(matches(isDisplayed()));

    onView(withText("YES")).perform(click());

    onView(withText("YES")).check(doesNotExist());
    onView(withId(R.id.galleryDetailImage)).check(matches(isDisplayed()));
  }
